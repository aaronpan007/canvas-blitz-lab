...
import { PORTRAIT_TEMPLATES } from "../../data/portraitTemplates";
import StyleCard from "./components/StyleCard";
import { STYLE_THUMBS } from "./styleAssets";
...
export const PortraitPage: React.FC<PortraitPageProps> = ({ onPromptSelect, images, onImageUpdate }) => {
  ...
  const handleGenerate = async () => {
    try {
      ...
      if (contentType.includes('application/json')) {
        ...
        console.error('[PORTRAIT] Non-JSON response:', text.slice(0, 500));
        throw new Error(`API returned non-JSON (${res.status}). Maybe proxy/route misconfigured.`);
      }
      
      console.log("[PORTRAIT] api resp:", data);
  ...
  return (
    <div className="mx-auto max-w-[1120px] px-6 pt-12 pb-16">
      ...
      <header className="text-center mb-8">
        <h1 className="text-2xl font-semibold text-white">Portrait Generator</h1>
        <p className="text-white/60 text-sm mt-2">Create stunning AI-generated portraits with custom styles</p>
      </header>
      ...
      <section className="mb-8">
        <h3 className="text-white/90 font-semibold text-lg mb-4">Choose Style</h3>
        ...
        <div className="flex justify-center">
          <button
            ...
            className={cn(
              ...
              "px-8 py-4 text-base lg:text-lg font-semibold",
              ...
              >
                ...
                <div className="p-4">
                  <div className="text-sm font-medium text-white">
                    {STYLES.find(s => s.id === result.styleId)?.label || result.styleId}
                  </div>
